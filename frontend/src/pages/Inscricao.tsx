...
interface FormConfigUpdateDetail {
...
const Inscricao = () => {
  ...
  useEffect(() => {
    const handleConfigUpdate = (event: Event) => {
      const customEvent = event as CustomEvent<FormConfigUpdateDetail>;
      console.log('🔄 Configuração do formulário foi atualizada!', customEvent.detail);
    ...
    if (validateStep(currentStep)) {
      ...
      if (currentStep === 1) {
        ...
        try {
          ...
          const fallbackMessage = getApiErrorMessage(error, "Não foi possível validar seus dados. Tente novamente.");
          toast({
            ...
            <div className="mt-6 grid gap-2" style={{ gridTemplateColumns: `repeat(${formData.menor_idade ? totalSteps : totalSteps - 1}, minmax(0, 1fr))` }}>
              {Array.from({ length: totalSteps }, (_, i) => i + 1)
                ...
                .map((step) => (
                  <div
                    key={step}
                    className={`flex flex-col items-center gap-2 ${step === currentStep ? 'scale-105' : ''
                  ...
                  >
                    ...
                    <span className={`text-xs text-center font-medium hidden sm:block ${step === currentStep ? 'text-primary' : 'text-muted-foreground'
                      }`}>
                      {getStepTitle(step)}
                    </span>
                  </div>
